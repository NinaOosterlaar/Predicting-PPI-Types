...
def help():
    return run(f"""
        SELECT s_name_1, s_name_2
        FROM ppi, gene
        WHERE ppi.ppi_type_id = 1
        AND (ppi.s_name_1 = gene.s_name OR ppi.s_name_2 = gene.s_name)
        AND ppi.confidence_score >= 700
        AND gene.organism_id = 3;
        """)

            
if __name__ == '__main__':
    # pos_interactions = retrieve_interactions(1, 901)
    # pos_interactions2 = retrieve_interactions(1, 900)
    # neg_interactions = retrieve_interactions(2, 0)
    # no_action = retrieve_interactions(9, 0)
    # neg_interactions = set(neg_interactions)
    # print(len(neg_interactions), len(no_action))
    # print(len(set(neg_interactions).intersection(set(no_action))))
    # print(len(set(pos_interactions).intersection(set(neg_interactions))))
    # print(len(pos_interactions), len(pos_interactions2))
    ...
    # print(len(set(pos_interactions).intersection(set(neg_interactions))))
    # print(len(retrieve_interactions(1, 900)))
    print(len(help()))
    print(len(set(help())))
