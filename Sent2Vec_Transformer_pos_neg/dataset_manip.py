...
class Dataset_stringDB(torch.utils.data.Dataset):
    ...
    def __init__(self, all_ppi, labels, protein_go_anno,  go_embed_dict, shuffle, go_id_dict_pth):
...
def get_dataset_split_stringDB(confidence_score_pos, confidence_score_neg, go_embed_pth, shuffle, ratio = [0.8, 0.2, 0], organism = [0], intr_set_size_filter = [0,500], max_intr_size = None):
    ...
    ppi_poz, updated_protein_go_anno = filter_interactions(ppi_poz, protein_go_anno, go_embed_dict, intr_set_size_filter, max_intr_size)
    ppi_neg, updated_protein_go_anno = filter_interactions(ppi_neg, updated_protein_go_anno, go_embed_dict, intr_set_size_filter, max_intr_size)
    ...
    full_dataset = Dataset_stringDB(all_ppi, labels, updated_protein_go_anno, go_embed_dict, shuffle)
...
def get_dataset_split_stringDB2(confidence_score_pos, confidence_score_neg, go_embed_pth, go_id_dict_pth, shuffle, filter_sec, ratio = [0.8, 0.2, 0], organism = [0], intr_set_size_filter = [0,500], max_intr_size = None, new_thingy = None):
    ...
    count = 0
    for go in filter_sec:
        if go not in go_embed_dict.keys():
            count += 1
    print(len(go_embed_dict.keys()))
    print(count, "!")   
    ...
    random.shuffle(full_dataset)
    all_ppi, labels = zip(*full_dataset)
    full_dataset = Dataset_stringDB(all_ppi, labels, updated_protein_go_anno, go_embed_dict, shuffle, go_id_dict_pth)
...
if __name__ == '__main__':
    print(len(json.load(open('Sent2Vec_Transformer_pos_neg\datasets\sentence_embedding.json'))))
