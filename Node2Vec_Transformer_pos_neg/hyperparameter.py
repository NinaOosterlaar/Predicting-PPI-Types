...
def train_epoch(MODEL_SIZE, NR_HEADS, NR_LAYERS, DROPOUT, SIZE_FF, LR, confidence_score_pos, confidence_score_neg, go_id_dict_pth, go_embed_pth, shuffle, name, params):
    N_EPOCHS = [1, 1, 1]
...
def train_heads(MODEL_SIZE, N_EPOCH, NR_LAYERS, DROPOUT, SIZE_FF, LR, confidence_score_pos, confidence_score_neg, go_id_dict_pth, go_embed_pth, shuffle, name, params):
    changeable= [4, 8, 16]
...
def train_layers(MODEL_SIZE, N_EPOCH, NR_HEADS, DROPOUT, SIZE_FF, LR, confidence_score_pos, confidence_score_neg, go_id_dict_pth, go_embed_pth, shuffle, name, params):
...
def train_dropout(MODEL_SIZE, N_EPOCH, NR_HEADS, NR_LAYERS, SIZE_FF, LR, confidence_score_pos, confidence_score_neg, go_id_dict_pth, go_embed_pth, shuffle, name, params):
...
def train_size_ff(MODEL_SIZE, N_EPOCH, NR_HEADS, NR_LAYERS, DROPOUT, LR, confidence_score_pos, confidence_score_neg, go_id_dict_pth, go_embed_pth, shuffle, name, params):
...
def train_LR(MODEL_SIZE, N_EPOCH, NR_HEADS, NR_LAYERS, DROPOUT, SIZE_FF, confidence_score_pos, confidence_score_neg, go_id_dict_pth, go_embed_pth, shuffle, name, params):
...
def train_embedding(N_EPOCH, NR_HEADS, NR_LAYERS, DROPOUT, SIZE_FF, confidence_score_pos, confidence_score_neg, shuffle, name, params):
    changeable= [32, 64, 128]
    
    
    total = len(changeable)
    
    Parallel(n_jobs=-1)(
        delayed(create_embedding)(
           N_EPOCH, MODEL_SIZE, NR_HEADS, NR_LAYERS, DROPOUT, SIZE_FF, LR, params, name, count, total, confidence_score_pos, confidence_score_neg, go_id_dict_pth, shuffle
        ) for count, MODEL_SIZE in enumerate(changeable)
    )
    
def create_embedding(N_EPOCH, MODEL_SIZE, NR_HEADS, NR_LAYERS, DROPOUT, SIZE_FF, LR, params, name, count, total, confidence_score_pos, confidence_score_neg, go_id_dict_pth, shuffle):
    global EMB_DIM
    EMB_DIM = MODEL_SIZE
    # paths
    go_id_dict_pth = "datasets/go_id_dict"
    go_name_space_dict_pth = "datasets/go_namespace_dict"
    go_embed_pth = f"datasets/go-terms-{MODEL_SIZE}.emd"
    
    # get dataset
    train_set, valid_set, test_set, all_dataset = get_dataset_split_stringDB(confidence_score_pos, confidence_score_neg, go_id_dict_pth, 
                                                                                go_embed_pth, shuffle, "", go_name_space_dict_pth=go_name_space_dict_pth, 
                                                                                ratio = [0.8, 0, 0.2], stringDB = False)
    cross_train(N_EPOCH, MODEL_SIZE, NR_HEADS, NR_LAYERS, DROPOUT, SIZE_FF, LR, train_set, test_set, params, name, count, total)
...
if __name__ == '__main__':
    ...
    # train_epoch(MODEL_SIZE, NR_HEADS, NR_LAYERS, DROPOUT, SIZE_FF, LR, confidence_score_pos, confidence_score_neg, go_id_dict_pth, go_embed_pth, shuffle, name, params)
    # train_heads(MODEL_SIZE, N_EPOCHS, NR_LAYERS, DROPOUT, SIZE_FF, LR, confidence_score_pos, confidence_score_neg, go_id_dict_pth, go_embed_pth, shuffle, name, params)
    # train_layers(MODEL_SIZE, N_EPOCHS, NR_HEADS, DROPOUT, SIZE_FF, LR, confidence_score_pos, confidence_score_neg, go_id_dict_pth, go_embed_pth, shuffle, name, params)
    # train_dropout(MODEL_SIZE, N_EPOCHS, NR_HEADS, NR_LAYERS, SIZE_FF, LR, confidence_score_pos, confidence_score_neg, go_id_dict_pth, go_embed_pth, shuffle, name, params)
    # train_size_ff(MODEL_SIZE, N_EPOCHS, NR_HEADS, NR_LAYERS, DROPOUT, LR, confidence_score_pos, confidence_score_neg, go_id_dict_pth, go_embed_pth, shuffle, name, params)
    # train_LR(MODEL_SIZE, N_EPOCHS, NR_HEADS, NR_LAYERS, DROPOUT, SIZE_FF, confidence_score_pos, confidence_score_neg, go_id_dict_pth, go_embed_pth, shuffle, name, params)
    train_embedding(N_EPOCHS, NR_HEADS, NR_LAYERS, DROPOUT, SIZE_FF, confidence_score_pos, confidence_score_neg, shuffle, name, params)
