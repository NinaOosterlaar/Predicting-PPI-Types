...
from obtain_data import retrieve_interactions, create_gene_GO_dict, delete_duplicates
...
def get_dataset_split_stringDB(interactions, go_id_dict_pth, go_embed_pth, shuffle, aliases_path = "", test_size = 0.2,\
                               stringDB = True, go_name_space_dict_pth = "datasets\go_namespace_dict", 
                               go_filter = "ALL", intr_set_size_filter = [0,500], max_intr_size = None, no_actions = True):
    ...
    for c, ppi in enumerate(all_ppi_list):
        print("Interactions of type ", c, ": ", len(ppi))
    ...
    if no_actions:
        ...
        all_ppi_list[2] = random.sample(all_ppi_list[2], length)   
     
    protein_go_anno = create_gene_GO_dict()
    
    for c, ppi in enumerate(all_ppi_list):
        all_ppi_list[c], protein_go_anno = filter_interactions(ppi, protein_go_anno, go_id_dict_pth, aliases_path, stringDB, go_name_space_dict_pth, go_filter, intr_set_size_filter, max_intr_size)
    ...
    for ppi in all_ppi:
        embedding, proteins = get_embedded_proteins(protein_go_anno, go_id_dict, go_emb_dict, shuffle, ppi[0], ppi[1])
