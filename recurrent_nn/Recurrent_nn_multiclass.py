...
from dataset_manip_multiclass import get_dataset_split_stringDB
...
def create_rnn_model(emb_dim, num_classes, summary=False):
    model = Sequential()
...
def obtain_data(interactions, go_id_dict_pth, go_embed_pth, shuffle, test_size):
    # get dataset
    train_embeddings, test_embeddings, train_labels, test_labels = get_dataset_split_stringDB(interactions, go_id_dict_pth, 
                                                                             go_embed_pth, shuffle, "", test_size)
    return train_embeddings, test_embeddings, train_labels, test_labels


def cross_train(c_fold, epoch, dimension, num_classes, train_embeddings, test_embeddings, train_labels, test_labels):
    ...
    for i in range(c_fold):
        ...
        # Train the model
        model = create_rnn_model(dimension, num_classes)
...
def main():
    ...
    c_fold = 2
    epoch = 3
    interactions = [(1, 980), (2, 970), (9, 0)]
    go_id_dict_pth = "datasets\go_id_dict"
    go_embed_pth = f"datasets\go-terms-{dim}.emd"
    test_size = 0.2
    train_embeddings, test_embeddings, train_labels, test_labels = obtain_data(interactions, go_id_dict_pth, go_embed_pth, shuffle, test_size)
    cross_train(c_fold, epoch, dim, len(interactions), train_embeddings, test_embeddings, train_labels, test_labels)
...
if __name__ == '__main__':
    ...
    main()
    with open("multiclass_models/31-05-2023-simple_rnn_result", "rb") as f:
